...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String? _lastSyncTime; // ISO string, dari SharedPreferences
  ...
  Future<void> _loadLastSyncTime() async {
    ...
    if (mounted) {
      setState(() => _lastSyncTime = prefs.getString('last_sync_time'));
    }
  }
  ...
  Future<void> _syncNow() async {
    ...
    try {
      ...
      if (result.synced > 0) {
        ...
        // Reload list agar synced=1 terefleksi
        await context.read<PengunjungProvider>().loadAll();
        _showSnack('✓ ${result.synced} record berhasil disinkronkan', Colors.green.shade600);
      } else {
        _showSnack('Semua data sudah ter-sinkronkan', Colors.blue.shade600);
      }
    } on SyncException catch (e) {
      if (mounted) _showSnack(e.message, Colors.orange.shade700);
    } catch (e) {
      if (mounted) _showSnack('Gagal sinkronisasi. Coba lagi.', Colors.red.shade600);
      debugPrint('[Sync] $e');
    } finally {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── Sync section ─────────────────────────────
              Consumer<PengunjungProvider>(
                builder: (ctx, provider, _) {
                  final unsyncedCount =
                      provider.list.where((p) => p.synced == 0).length;
                  return _SyncCard(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            height: 48,
            child: ElevatedButton.icon(
              onPressed: isSyncing || !isConfigured ? null : onSync,
              icon: isSyncing
                  ? const SizedBox(
                      width: 18,
                      height: 18,
          ...
          const SizedBox(height: 8),
          Text(
            'Data di-backup ke Google Sheets. '
            'Sync otomatis berjalan saat app dibuka dengan koneksi internet.',
            textAlign: TextAlign.center,
            style:
